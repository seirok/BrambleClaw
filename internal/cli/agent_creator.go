...
func NewAgentCreator(cfg *config2.Config, cfgPath string) *AgentCreator {
	return &AgentCreator{
...
func (c *AgentCreator) Run() error {
	...
	// 9. 创建 Agent 配置
	agentConfig := config2.AgentConfig{
		Name:        name,
...
func (c *AgentCreator) promptLLMConfig() (config2.LLMConfig, error) {
	// 如果有全局配置，使用全局配置作为默认值
...
func (c *AgentCreator) validateLLM(llmConfig config2.LLMConfig) error {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	result, err := config2.ValidateLLMConfig(ctx, llmConfig)
	if err != nil {
