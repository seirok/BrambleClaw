	...
	cfg.Agents = []structs.AgentConfig{
		{
			...
			Tools:       []string{"web_search", "shell", "filesystem", "url_parse", "glob", "grep", "read", "write", "grant_permission", "cron"},
			MaxHistory:  50,
			Enabled:     true,
		},
		{
			Name:        "daemon",
			Description: "Background agent for executing scheduled tasks",
			LLM:         structs.DefaultLLMConfig(),
			Tools:       []string{"web_search", "shell", "filesystem", "url_parse", "glob", "grep", "read", "write", "grant_permission", "cron"},
			MaxHistory:  50,
			Enabled:     true,
