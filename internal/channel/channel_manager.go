...
func (m *ChannelManager) Initialize(ctx context.Context, cfg any) error {
	...
	// 初始化 Telegram 通道
	telegramCfg := cfgObj.Channels.Telegram
	if telegramCfg.Enabled == true {
		telegramChannel, err := NewTelegramChannel(telegramCfg, m.msgBus)
		if err != nil {
			logger.L().Error().Err(err).Msg("Failed to create Telegram channel")
		} else {
			if err = m.channelRegistry.Register(ctx, "telegram", telegramChannel); err != nil {
				logger.L().Error().Err(err).Msg("Failed to register Telegram channel")
			}
		}
	} else {
